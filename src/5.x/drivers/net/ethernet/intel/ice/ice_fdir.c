			...
			u16 idx, u16 offset, bool tun)
{
	...
	/* copy outer packet */
	memcpy(pkt, ice_fdir_pkt[idx].tun_pkt, pos);

	/* copy l2tpv2 packet common header */
	memcpy(pkt + pos, &l2tpv2_data->flags_version,
	...
	if (ice_fdir_pkt[idx].tun_pkt_len > offset) {
		/* copy ppp packet */
		memcpy(pkt + pos,
		       ice_fdir_pkt[idx].tun_pkt + offset - ICE_PPP_PKT_LENGTH,
		       ICE_PPP_PKT_LENGTH);
		pos += ICE_PPP_PKT_LENGTH;

		/* copy inner packets */
		if (tun) {
			memcpy(pkt + pos, ice_fdir_pkt[idx].tun_pkt + offset,
			  ...
			  u8 *pkt, bool frag, bool tun)
{
	...
	if (!tun) {
		...
		case ICE_FLTR_PTYPE_NONF_IPV6_L2TPV2_PPP:
			...
			break;
		default:
			memcpy(pkt, ice_fdir_pkt[idx].pkt,
