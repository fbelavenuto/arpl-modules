...
static void be_eq_notify(struct be_adapter *adapter, u16 qid,
			 bool arm, bool clear_int, u16 num_popped, u32 r2i_dly)
{
	u32 val = 0;
	...
	val |= r2i_dly << DB_EQ_R2I_DLY_SHIFT;
...
static int be_dev_mac_add(struct be_adapter *adapter, u8 *mac)
{
	...
	for (i = 0; i < adapter->uc_macs; i++) {
		if (ether_addr_equal(&adapter->uc_list[i * ETH_ALEN], mac)) {
...
static int be_mac_addr_set(struct net_device *netdev, void *p)
{
	...
	memcpy(adapter->dev_mac, addr->sa_data, netdev->addr_len);
...
void be_link_status_update(struct be_adapter *adapter, u8 link_status)
{
	...
	dev_info(&adapter->pdev->dev, "%s: Link is %s\n", netdev->name,
		 link_status ? "Up" : "Down");
}

static inline int be_gso_hdr_len(struct sk_buff *skb)
{
#ifdef VXLAN_NDO_defined
				       ...
				       struct be_wrb_params *wrb_params)
{
	...
	if (skb_is_gso(skb)) {
		...
		if (lancer_chip(adapter) &&
			...
			wrb_params->lso_mss = LANCER_TSO_MIN_MSS;
		else
			 ...
			 struct be_wrb_params *wrb_params, struct sk_buff *skb)
{
	...
	SET_TX_WRB_HDR_BITS(crc, hdr, BE_WRB_F_GET(wrb_params->features, CRC));
	...
	SET_TX_WRB_HDR_BITS(lso, hdr, BE_WRB_F_GET(wrb_params->features, LSO));
	SET_TX_WRB_HDR_BITS(lso6, hdr,
			    BE_WRB_F_GET(wrb_params->features, LSO6));
	SET_TX_WRB_HDR_BITS(lso_mss, hdr, wrb_params->lso_mss);

	/* Hack to skip HW VLAN tagging needs evt = 1, compl = 0
	 * When this hack is not needed, the evt bit or mgmt bit is set
	 * while ringing DB
	 */
 ...
 */
static void be_xmit_restore(struct be_adapter *adapter, struct be_tx_obj *txo,
			    u32 head, bool map_single, u32 copied)
{
	struct device *dev = &adapter->pdev->dev;
	struct be_queue_info *txq = &txo->q;
	struct be_eth_wrb *wrb;
					     ...
					     struct be_wrb_params *wrb_params)
{
	...
	if (qnq_async_evt_rcvd(adapter) && adapter->pvid) {
		...
		/* FW workaround to set skip_hw_vlan = 1, informs the FW to
		 * skip VLAN insertion
		 */
...
static u16 be_get_vlan(struct be_adapter *adapter,
		       struct sk_buff *skb, u16 vlan_id)
{
...
static void be_unlearn_mac(struct be_adapter *adapter,
			   struct sk_buff *skb, u16 rxcp_vlan_id)
{
	...
	vlan_id = be_get_vlan(adapter, skb, rxcp_vlan_id);
	hashidx = be_mac_hash(ethhdr->h_source);
	head = &adapter->mac_table[hashidx];

	rcu_read_lock_bh();
	hlist_for_each_entry_rcu_bh(node, head, list) {
		if (be_mac_vlan_matches(node, ethhdr, vlan_id)) {
			spin_lock_bh(&adapter->mac_hash_lock);
				  ...
				  u8 hashidx)
{
	...
	vlan_id = be_get_vlan(adapter, skb, 0);
 ...
 */
static void be_learn_mac(struct be_adapter *adapter, struct sk_buff *skb)
{
	struct ethhdr *ethhdr = (struct ethhdr *)skb->data;
	u8 hashidx;
						  ...
						  *wrb_params)
{
	 ...
	 */
	if (!BE_WRB_F_GET(wrb_params->features, VLAN_SKIP_HW) &&
	 ...
	 */
	if (!BE_WRB_F_GET(wrb_params->features, VLAN_SKIP_HW) &&
	    be_ipv6_tx_stall_chk(adapter, skb)) {
		skb = be_insert_vlan_in_pkt(adapter, skb, wrb_params);
		if (unlikely(!skb))
			return NULL;
	}
					   ...
					   struct be_wrb_params *wrb_params)
{
	...
	/* Lancer, SH ASICS and BE3 ASIC in SRIOV mode has a bug wherein
 	 * packets that are 32 bytes or less may cause a transmit stall
 	 * on that port. So the work-around is to pad short packets 
 	 * (<= 32 bytes) to a 36-byte length.
	 */
	if (skb->len <= 32) {
		if (skb_padto(skb, 36))
			return NULL;
...
static void be_xmit_flush(struct be_adapter *adapter, struct be_tx_obj *txo)
{
	...
	u16 wrb_cnt = txo->pend_wrb_cnt ? txo->pend_wrb_cnt :
							txo->last_req_wrb_cnt;
			       ...
			       struct sk_buff **skb)
{
	...
	if (!is_be_os2bmc_enabled(adapter))
		goto done;
...
static netdev_tx_t be_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	 ...
	 * But ensure that we don't batch too many.
	 */
	if (atomic_read(&txq->used) > START_BATCHING_WM &&
	    txo->pend_wrb_cnt < MAX_BATCH_SIZE)
		flush = false;
...
static vid_return_t be_vlan_add_vid(struct net_device *netdev, __be16 proto,
				    u16 vid)
{
	struct be_adapter *adapter = netdev_priv(netdev);
	int status = 0;
...
ret:
	mutex_unlock(&adapter->rx_filter_lock);
	ndo_vlan_return(status);
}

static vid_return_t be_vlan_rem_vid(struct net_device *netdev, __be16 proto,
				    u16 vid)
{
	struct be_adapter *adapter = netdev_priv(netdev);
	int status = 0;
...
static void be_set_mc_list(struct be_adapter *adapter)
{
	...
	if (adapter->update_mc_list) {
		/* cache the list in adapter array */
		netdev_for_each_mc_addr(ha, netdev) {
			ether_addr_copy(adapter->mc_list + i, ha->DMI_ADDR);
			i += ETH_ALEN;
		}
...
static int be_uc_mac_add(struct be_adapter *adapter, int uc_idx)
{
	...
	if (ether_addr_equal(&adapter->uc_list[uc_idx * ETH_ALEN],
			     adapter->dev_mac)) {
		adapter->pmac_id[uc_idx + 1] = adapter->pmac_id[0];
		return 0;
	}

	return be_cmd_pmac_add(adapter, &adapter->uc_list[uc_idx * ETH_ALEN],
			       adapter->if_handle,
...
static void be_set_uc_list(struct be_adapter *adapter)
{
	...
	if (adapter->update_uc_list) {
		...
		netdev_for_each_uc_addr(ha, netdev) {
			ether_addr_copy(adapter->uc_list + i, ha->addr);
			i += ETH_ALEN;
		}
			    ...
			    struct ifla_vf_info *vi)
{
	...
	vi->vf = vf;
#ifdef NDO_VF_RATE_defined
	vi->max_tx_rate = vf_cfg->tx_rate;
	vi->min_tx_rate = 0;
#else
	vi->tx_rate = vf_cfg->tx_rate;
#endif /* NDO_VF_RATE_defined */
...
#ifdef NDO_VF_SPOOFCHK_defined
	vi->spoofchk = adapter->vf_cfg[vf].spoofchk;
#endif /* NDO_VF_SPOOFCHK_defined */
	return 0;
}
...
config_qos:
	...
	return 0;
err:
	dev_err(dev, "Tx-rate setting of %dMbps on VF%d failed: %#x\n",
		max_tx_rate, vf, status);
	return -EIO;
}
				...
				int link_state)
{
	...
	status = be_cmd_set_link_tracking(adapter, link_state, vf + 1);
...
static int be_get_new_eqd(struct be_eq_obj *eqo)
{
	...
	if (!aic->jiffies || time_before(now, aic->jiffies) ||
	    rxpkts < aic->rx_pkts_prev ||
	    txpkts < aic->tx_reqs_prev) {
		be_aic_update(aic, rxpkts, txpkts, now);
		return aic->prev_eqd;
	}

	delta = jiffies_to_msecs(now - aic->jiffies);
	/* Too early interrupt calculation.
	 * Defer EQD calculation
	 */
	if (delta == 0)
		return aic->prev_eqd;
...
static u32 sh_get_r2i_delay(struct be_eq_obj *eqo)
{
	struct be_adapter *adapter = eqo->adapter;
...
static struct be_rx_page_info *get_rx_page_info(struct be_rx_obj *rxo)
{
	...
	if (rx_page_info->last_frag) {
		dma_unmap_page(dev, rx_page_info->dma_addr,
			       adapter->big_page_size, DMA_FROM_DEVICE);
		rx_page_info->last_frag = false;
	} else {
		dma_sync_single_for_cpu(dev, rx_page_info->dma_addr,
				...
				struct be_rx_compl_info *rxcp)
{
	...
	if (adapter->flags & BE_FLAGS_MAC_LEARNING_INITIALIZED)
		be_unlearn_mac(adapter, skb, rxcp->vlan_tag & VLAN_VID_MASK);
				 ...
				 struct be_rx_compl_info *rxcp)
{
	...
	rxcp->tunneled = GET_RX_COMPL_V1_BITS(tunneled, compl);
}
...
static void be_post_rx_frags(struct be_rx_obj *rxo, gfp_t gfp, u32 frags_needed)
{
	...
	for (posted = 0; posted < frags_needed && !page_info->page; posted++) {
		if (!pagep) {
			...
			get_page(pagep);
			page_offset += adapter->rx_frag_size;
		}
		...
		if ((page_offset + adapter->rx_frag_size +
		     adapter->rx_frag_size) > adapter->big_page_size) {
			pagep = NULL;
			page_info->last_frag = true;
			page_info->dma_addr = page_dmaaddr;
		} else {
			page_info->dma_addr = frag_dmaaddr;
		}
	...
	if (pagep) {
		...
		prev_page_info->dma_addr = page_dmaaddr;
	}
...
static int be_rx_cqs_create(struct be_adapter *adapter)
{
	...
	adapter->num_rss_qs = min(adapter->num_evt_qs, adapter->cfg_num_rx_irqs);
...
static u16 be_get_roce_vec_share(struct be_adapter *adapter, int max_vec) 
{
	/* RDMA profile needs half the vectors. Other profiles that use
	 * roce driver interface need just one vector.
	 */ 
	if (adapter->function_mode & RDMA_ENABLED)
		return max_vec/2;
	else if (adapter->function_mode & SURF_ENABLED)
		return 1;
	else
		return 0;
}
...
{
	struct device *dev = &adapter->pdev->dev;
	u16 i, num_vec, max_roce_eqs;
	int status;

	max_roce_eqs = be_max_func_eqs(adapter) - be_max_nic_eqs(adapter);

	num_vec = be_max_any_irqs(adapter) +
		  min_t(u16, max_roce_eqs, num_online_cpus());
	...
	if (num_vec > MIN_MSIX_VECTORS)
		adapter->num_msix_roce_vec =
				be_get_roce_vec_share(adapter, num_vec);
	adapter->num_msix_vec = num_vec - adapter->num_msix_roce_vec;

	dev_info(dev, "enabled %d (out of %d) MSI-x vector(s) for NIC\n",
...
static u16 be_select_queue(struct net_device *netdev, struct sk_buff *skb,
			   void *accel_priv, select_queue_fallback_t fallback)
{
	...
	if (adapter->flags & BE_FLAGS_PG_PFC) {
		prio = (skb_vlan_tag_get(skb) & VLAN_PRIO_MASK) >>
			VLAN_PRIO_SHIFT;
...
static int be_close(struct net_device *netdev)
{
	...
	if (be_physfn(adapter))
		be_cmd_set_link_quiesce(adapter, true);
...
static int be_rx_qs_create(struct be_adapter *adapter)
{
	...
	if (be_multi_rxq(adapter)) {
		...
		netdev_rss_key_fill(rss_hkey, RSS_HASH_KEY_LEN);
		rc = be_cmd_rss_config(adapter, rss->rsstable, rss->rss_flags,
				       RSS_INDIR_TABLE_LEN, rss_hkey);
		if (rc) {
			rss->rss_flags = RSS_ENABLE_NONE;
			return rc;
		}
	...
	/* Post 1 less than full capacity of rx buffers,
	 * to avoid a ring empty condition in the HW
	 */
	for_all_rx_queues(adapter, rxo, i)
...
static int be_open(struct net_device *netdev)
{
	...
	if (be_physfn(adapter))
		be_cmd_set_link_quiesce(adapter, false);
...
static void be_vf_clear(struct be_adapter *adapter)
{
	...
	/* In older kernels(RHEL 6.4, SLES11SP3 and earlier) which do not
	 * support pci_sriov_set_totalvfs(), unloading driver when VFs are
	 * assigned to VMs results in abnormal behavior of Max VFs count.
	 * Hence, in those kernels wait to unload until all vfs are detached.
	 */
...
static void be_disable_vxlan_offloads(struct be_adapter *adapter)
{
	...
	BE_NETDEV_HW_FEATURES(netdev) &= ~(NETIF_F_GSO_UDP_TUNNEL);
...
static int be_clear(struct be_adapter *adapter)
{
	...
	if (adapter->config_workq) {
		...
		adapter->config_workq = NULL;
	}

	if ((adapter->mc_type == NPAR) || (adapter->mc_type == UMC))
...
static int be_vf_setup(struct be_adapter *adapter)
{
	...
	for_all_vfs(adapter, vf_cfg, vf) {
		 ...
		 * max_rate = 0, don't want any tx rate limiting.
		 */
		if (!old_vfs)
			be_cmd_config_qos(adapter, 0, 10000, vf + 1);
		...
		if (!old_vfs || be_error_recovering(adapter)) {
			...
			be_cmd_set_link_tracking(adapter,
	...
	if (BE3_chip(adapter)) {
		/* On BE3, enable VEB when only when SRIOV is enabled */
...
{
	if (function_mode & QNQ_MODE && function_mode & VNIC_MODE)
		return vNIC1;
			      ...
			      struct be_resources *res)
{
	...
	if (BE2_chip(adapter) || use_sriov || (adapter->port_num > 1) ||
	    be_virtfn(adapter) || (be_is_mc(adapter) &&
...
static int be_get_resources(struct be_adapter *adapter)
{
	...
	dev_info(dev, "Max: uc-macs %d, mc-macs %d, vlans %d\n",
		 be_max_uc(adapter), be_max_mc(adapter), be_max_vlans(adapter));
...
static void be_setup_mac_learning(struct be_adapter *adapter)
{
	...
	/* Mac learning async event not supported in UMC mode */
	if (adapter->mc_type == UMC)
...
static int be_if_create(struct be_adapter *adapter)
{
	...
	adapter->mc_list = kcalloc(be_max_mc(adapter), ETH_ALEN, GFP_KERNEL);
	if (!adapter->mc_list)
		return -ENOMEM;

	adapter->uc_list = kcalloc(be_max_uc(adapter), ETH_ALEN, GFP_KERNEL);
	if (!adapter->uc_list)
		return -ENOMEM;
	return 0;
}
...
static int be_setup(struct be_adapter *adapter)
{
	...
	/* Need to invoke this cmd first to get the PF and VF Number */
	if (!BEx_chip(adapter)) {
	...
	if (!adapter->config_workq) {
		status = -ENOMEM;
		dev_err(&adapter->pdev->dev,
			"Cannot create be_config workqueue\n");
		goto err;
	}
	...
	if (be_physfn(adapter))
		be_cmd_set_link_tracking(adapter, IFLA_VF_LINK_STATE_AUTO, 0);
	...
	if (!status && be_pause_autoneg_supported(adapter)) {
		...
		adapter->phy.autoneg = 1;
	}

	if (be_physfn(adapter) && !lancer_chip(adapter)) {
		if ((adapter->phy.phy_type == PHY_TYPE_TN_8022) &&
		       ...
		       struct ifreq *ifr, int cmd)
{
	...
	case SIOCDEVPRIVATE:
		...
		va = dma_zalloc_coherent(dev, req_size, &dma, GFP_KERNEL);
		if (!va)
			return -ENOMEM;
		...
		ret = be_cmd_pass_ext_ioctl(adapter, dma, req_size, va);
		if (ret)
			break;
	...
	if (va)
		dma_free_coherent(dev, req_size, va, dma);
...
int be_load_fw(struct be_adapter *adapter, u8 *fw_file)
{
	...
	if (!netif_running(adapter->netdev)) {
		dev_err(dev, "Firmware load not allowed (interface is down)\n");
		return -ENETDOWN;
	}

	status = request_firmware(&fw, fw_file, dev);
	if (status)
		return be_cmd_status(status);
	...
	dev_info(dev, "Flashing firmware file %s\n", fw_file);

	if (lancer_chip(adapter))
		status = lancer_fw_download(adapter, fw, &flash_cmd);
	else
		status = be_fw_download(adapter, fw, &flash_cmd);
...
static void be_cfg_vxlan_port(struct net_device *netdev, sa_family_t sa_family,
			      __be16 port, void (*func)(struct work_struct *))
{
	struct be_adapter *adapter = netdev_priv(netdev);
	struct be_cmd_work *cmd_work;
...
static void be_del_vxlan_port(struct net_device *netdev, sa_family_t sa_family,
			      __be16 port)
{
	be_cfg_vxlan_port(netdev, sa_family, port, be_work_del_vxlan_port);
}

static void be_add_vxlan_port(struct net_device *netdev, sa_family_t sa_family,
			      __be16 port)
{
	be_cfg_vxlan_port(netdev, sa_family, port, be_work_add_vxlan_port);
}

#ifdef VXLAN_NDO_FEATURES_CHK_defined
...
#endif	/* VXLAN_NDO_FEATURES_CHK_defined */
#endif /* VXLAN_NDO_defined */

#ifdef NDO_GET_PHYS_PORT_ID_defined
#ifdef NDO_GET_PHYS_PORT_ID_NEW
...
#endif
#if defined CONFIG_NET_RX_BUSY_POLL && !defined NETDEV_OPS_EXT_defined
...
static int be_netdev_init(struct net_device *netdev)
{
	...
	BE_NETDEV_HW_FEATURES(netdev) |= NETIF_F_SG | NETIF_F_TSO |
		NETIF_F_TSO6 | NETIF_F_IP_CSUM | NETIF_F_IPV6_CSUM |
		NETIF_F_RXCSUM | NETIF_F_HW_VLAN_CTAG_TX | NETIF_F_GRO;
	if ((be_if_cap_flags(adapter) & BE_IF_FLAGS_RSS))
		BE_NETDEV_HW_FEATURES(netdev) |= NETIF_F_RXHASH;

	netdev->features |= BE_NETDEV_HW_FEATURES(netdev) |
...
static void be_check_fw_health(struct be_adapter *adapter)
{
	struct be_mcc_obj *mcc_obj = &adapter->mcc_obj;
	int status;

	if (be_check_error(adapter, BE_ERROR_FW))
		return;

	/*  Ensure FW health check is invoked not more than once a second */
	if (jiffies_to_msecs(jiffies - mcc_obj->hb_sent_time) < 1000)
		return;

	/* Use the GET_CNTL_ADDITIONAL_ATTRIBUTES cmd as means to check
	 * FW health
	 */
	if (!mcc_obj->hb_sent) {
		if (rtnl_trylock()) {
			...
			/* Send the cmd only if there are no other cmds
			 * in pipeline
			 */
			if (atomic_read(&mcc_obj->q.used) == 0) {
				mcc_obj->hb_sent = true;
				status = be_cmd_check_fw_health(adapter);
				if (status)
					mcc_obj->hb_sent = false;
				else
					mcc_obj->hb_sent_time = jiffies;
			}
			mutex_unlock(&adapter->mcc_lock);
			rtnl_unlock();
		}
	} else {
		if (jiffies_to_msecs(jiffies - mcc_obj->hb_sent_time) >
		    BE_HEARTBEAT_TIMEOUT) {
			dev_err(&adapter->pdev->dev, "Error in FW/Card\n");
...
reschedule:
	queue_delayed_work(be_wq, &adapter->work, delay);
}
			   ...
			   char *buf)
{
	...
	return sprintf(buf, "%u\n",
		       adapter->hwmon_info.be_on_die_temperature * 1000);
}
...
static void be_register_hwmon_dev(struct be_adapter *adapter)
{
	struct device *dev = &adapter->pdev->dev;

	adapter->hwmon_info.hwmon_dev =
			devm_hwmon_device_register_with_groups(dev, DRV_NAME,
							       adapter,
							       be_hwmon_groups);

	if (IS_ERR(adapter->hwmon_info.hwmon_dev))
		adapter->hwmon_info.hwmon_dev = NULL;
}

void be_unregister_hwmon_dev(struct be_adapter *adapter)
{
#ifndef HWMON_DEV_REGISTER_WITH_GROUPS
	if (adapter->hwmon_info.hwmon_dev)
		devm_hwmon_device_unregister(adapter->hwmon_info.hwmon_dev);
#endif
}
...
static void be_remove(struct pci_dev *pdev)
{
	...
	if (be_physfn(adapter))
		be_unregister_hwmon_dev(adapter);
...
static char *port_speed_string(struct be_adapter *adapter)
{
	...
	if (speeds_supported & BE_SUPPORTED_SPEED_40GBPS)
		str = "40Gbps NIC ";
	else if (speeds_supported & BE_SUPPORTED_SPEED_20GBPS)
		str = "20Gbps NIC ";
...
static inline char *nic_name(struct pci_dev *pdev)
{
	switch (pdev->device) {
	case OC_DEVICE_ID1:
		dev_info(&pdev->dev, "%s: Obsolete, Unsupported Adapter\n",
			 BE_NAME);
		return OC_NAME;
	case OC_DEVICE_ID2:
		return OC_NAME_BE;
...
static int be_probe(struct pci_dev *pdev, const struct pci_device_id *pdev_id)
{
	 ...
	 */
#if !defined(PCI_FLAGS_ASSIGNED_defined)
	if (sriov_enabled(adapter))
	...
	be_schedule_err_detection(adapter, ERR_RECOVERY_DETECTION_DELAY);

	/* On Die temperature not supported for VF. */
	if (be_physfn(adapter))
		be_register_hwmon_dev(adapter);
...
int be_pci_sriov_configure(struct pci_dev *pdev, int num_vfs)
{
	...
	if (num_vfs == 0 && pci_vfs_assigned(pdev)) {
		dev_warn(&pdev->dev,
...
#ifdef PCI_DRIVER_RH_defined
static struct pci_driver_rh be_driver_rh = {
...
#if defined(SRIOV_CONFIGURE_defined) && !defined(PCI_DRIVER_RH_defined)
	.sriov_configure = be_pci_sriov_configure,
#else
#if defined(PCI_DRIVER_RH_defined)
	.rh_reserved = &be_driver_rh,
#endif
#endif
...
static int __init be_init_module(void)
{
	...
	if (!be_wq) {
		printk(KERN_WARNING DRV_NAME "workqueue creation failed\n");
		return -ENOMEM;
	}
	...
	if (rss_on_mc > 0) {
		pr_info(DRV_NAME " : Module param rss_on_mc is obsolete.");
		pr_info(DRV_NAME " : Ignoring rss_on_mc param setting.\n");
	}
