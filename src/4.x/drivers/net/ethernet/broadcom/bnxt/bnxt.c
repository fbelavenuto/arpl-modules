						   ...
						   gfp_t gfp)
{
	...
	*mapping = dma_map_single_attrs(&pdev->dev,
					data + bp->rx_dma_offset,
					bp->rx_buf_use_size, bp->rx_dir,
					DMA_ATTR_WEAK_ORDERING);
...
static int bnxt_alloc_stats(struct bnxt *bp)
{
	...
	if (BNXT_PF(bp) && bp->chip_num != CHIP_NUM_58700) {
		...
		memset(bp->hw_rx_port_stats, 0, bp->hw_port_stats_size);
...
static int __bnxt_hwrm_ptp_qcfg(struct bnxt *bp)
{
	...
	if (bp->hwrm_spec_code < 0x10801 || ptp)
		return 0;
	...
	for (i = 0, j = 0; i < bp->cp_nr_rings; i++) {
		...
		if (zalloc_cpumask_var(&irq->cpu_mask, GFP_KERNEL)) {
			...
			cpumask_set_cpu(cpumask_local_spread(i, numa_node),
					irq->cpu_mask);
#else
			rc = cpumask_set_cpu_local_first(i, numa_node,
							 irq->cpu_mask);
			if (rc) {
...
#if defined(CONFIG_ARM64) && defined(BNXT_TIMER_POLLED_VF)
static void bnxt_poll_vf(struct net_device *dev)
{
	struct bnxt *bp = netdev_priv(dev);
	int i;
...
#ifndef HAVE_MIN_MTU
	if (new_mtu < 60 || new_mtu > bp->max_mtu)
		return -EINVAL;
...
static int bnxt_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (ent->driver_data == NETXTREME_S_VF)
		bp->flags |= BNXT_FLAG_CHIP_SR;
	...
	bp->ulp_version = BNXT_ULP_VERSION;
