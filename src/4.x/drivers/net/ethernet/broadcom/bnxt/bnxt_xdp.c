...
void __bnxt_xmit_xdp_redirect(struct bnxt *bp, struct bnxt_tx_ring_info *txr,
...
void bnxt_tx_int_xdp(struct bnxt *bp, struct bnxt_napi *bnapi, int nr_pkts)
{
	...
	if (rx_doorbell_needed) {
		...
		bnxt_db_write(bp, rxr->rx_doorbell, DB_KEY_RX | tx_buf->rx_prod);
	}
}
		 ...
		 struct page *page, u8 **data_ptr, unsigned int *len, u8 *event)
{
	...
	case XDP_REDIRECT:
		 ...
		 * bnxt_en driver.
		 */
		err = xdp_do_redirect(bp->dev, &xdp, xdp_prog);
...
int bnxt_xdp_xmit(struct net_device *dev, struct xdp_buff *xdp)
{
	...
	len = xdp->data_end - xdp->data;
	mapping = dma_map_single(&pdev->dev, xdp->data, len, DMA_TO_DEVICE);

	if (dma_mapping_error(&pdev->dev, mapping))
		return -ENOMEM;

	__bnxt_xmit_xdp_redirect(bp, txr, mapping, len);
