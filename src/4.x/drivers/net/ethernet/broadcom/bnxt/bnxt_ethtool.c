...
static const struct {
	...
	BNXT_RX_STATS_EXT_ENTRY(link_down_counter),
	BNXT_RX_STATS_EXT_ENTRY(detect_continuous_pause_counter),
	BNXT_RX_STATS_EXT_ENTRY(resume_pause_counter),
};
			 ...
			 u32 *reg_buf)
{
	...
	for (i = 0; i < num_words; ) {
		...
		req.read_addr = cpu_to_le32(reg_off + i * 4 +
					    CHIMP_REG_VIEW_ADDR);
		words = num_words - i;
		words = min_t(u16, words, HWRM_DBG_REG_BUF_SIZE / 4);
		req.read_len32 = cpu_to_le32(words);
		rc = _hwrm_send_message(bp, &req, sizeof(req),
					HWRM_CMD_TIMEOUT);
		...
		for (j = 0; j < words; j++)
			reg_buf[i + j] = le32_to_cpu(dbg_reg_buf[j]);
...
static int bnxt_hwrm_rd_serdes(struct bnxt *bp, u8 serdes_type, u32 *reg_buf)
{
	struct hwrm_selftest_retrieve_serdes_data_input req = {0};
	struct hwrm_selftest_retrieve_serdes_data_output *resp =
		bp->hwrm_cmd_resp_addr;
	__le32 *dbg_reg_buf = (__le32 *)bp->hwrm_dbg_resp_addr;
	int timeout = HWRM_CMD_TIMEOUT;
	u16 offset = 0, total_len;
	int rc;

	if (bp->hwrm_spec_code < 0x10802)
		return -EOPNOTSUPP;
	if (!bp->hwrm_dbg_resp_addr)
		return -ENOMEM;
	bnxt_hwrm_cmd_hdr_init(bp, &req, HWRM_SELFTEST_RETRIEVE_SERDES_DATA, -1,
			       -1);
	req.resp_data_addr = cpu_to_le64(bp->hwrm_dbg_resp_dma_addr);
	req.data_len = cpu_to_le16(HWRM_DBG_REG_BUF_SIZE);
	req.flags = serdes_type;
	if (bp->test_info)
		timeout = bp->test_info->timeout;

	mutex_lock(&bp->hwrm_cmd_lock);
	do {
		u16 i, idx, copied;

		req.resp_data_offset = cpu_to_le32(offset);
		rc = _hwrm_send_message(bp, &req, sizeof(req), timeout);
		if (rc || resp->error_code) {
			rc = -EIO;
			break;
		}
		copied = le16_to_cpu(resp->copied_data_len);
		for (i = 0, idx = offset / 4; i < copied / 4; i++)
			reg_buf[idx + i] = le32_to_cpu(dbg_reg_buf[i]);
		offset += copied;
		total_len = le16_to_cpu(resp->total_data_len);
			  ...
			  void *_p)
{
	...
	regs->version = bp->reg_ver;
	bnxt_dbg_hwrm_rd_reg(bp, 0, BNXT_PXP_REG_LEN / 4, _p);

	if (bp->reg_len == BNXT_REG_BLOCK)
		return;

	bnxt_hwrm_rd_serdes(bp, BNXT_ETH_SERDES_TYPE, _p + BNXT_REG_BLOCK);
	bnxt_hwrm_rd_serdes(bp, BNXT_PCIE_SERDES_TYPE, _p + BNXT_REG_BLOCK +
			    BNXT_ETH_SERDES_REG_BLOCK);
}
...
void bnxt_ethtool_init(struct bnxt *bp)
{
	...
	if (!BNXT_PF(bp)) {
		bp->reg_len = 0;
	} else if (bp->hwrm_spec_code < 0x10802) {
		bp->reg_len = BNXT_REG_BLOCK;
	} else {
		bp->reg_len = BNXT_REG_BLOCK + BNXT_ETH_SERDES_REG_BLOCK +
			      BNXT_PCIE_SERDES_REG_BLOCK;
		bp->reg_ver = 1;
	}
